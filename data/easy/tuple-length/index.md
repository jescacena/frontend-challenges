...
Para obtener la longitud de una tupla como literal numerico (por ejemplo `4`), usa el acceso por indice sobre la propiedad `length`: `T["length"]`, y restringe `T` a `readonly any[]` para aceptar tuplas con `as const`.

## Problem

For a given tuple, you need to create a generic `Length` that picks the length of the tuple (as a specific number literal, not just `number`).
...
[03-tuple-length-challenge-1.ts](./03-tuple-length-challenge-1.ts)
