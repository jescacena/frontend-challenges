...
Para obtener el primer tipo de una tupla, haz pattern matching con tipos condicionales: si `T` encaja como `[infer F, ...any[]]`, entonces el primero es `F`; si no (tupla vacia), devuelve `never`.
...
3. If no (empty tuple `[]`), the pattern does not match -> return `never`
...
[04-first-of-array-challenge-2.ts](./04-first-of-array-challenge-2.ts)
