  ...
  while (i < expr.length) {
    ...
    if (ch >= "A" && ch <= "Z") {
      ...
      if (digitStart === i) {
        return {
          ok: false,
          error: `Invalid cell reference near: ${expr.slice(start, start + 3)}`,
        };
      }
...
export function toRpn(
  tokens: Token[],
): { ok: true; rpn: Token[] } | { ok: false; error: string } {
  const out: Token[] = [];
  const ops: Token[] = [];
  ...
  const popWhile = (incoming: Op) => {
    while (ops.length) {
      ...
      const shouldPop =
        topPrec > inPrec || (topPrec === inPrec && !isRightAssoc(incoming));
...
export function evalRpn(rpn: Token[], resolve: (id: CellId) => string): string {
  ...
  const parseNumeric = (
    v: string,
  ): { ok: true; n: number } | { ok: false; err: string } => {
    v = v.trim();
